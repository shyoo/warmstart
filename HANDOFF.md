...
- ⭐ **A reading is announced where it is *stored*** (2026-09-01, t86). The strip sat on a **3h-old** percentage across a resume, on an account the dispatch gate had just refreshed — the row was written and correct, and only 2 of the 4 callers that write one said so: the sweep, via its listener, and the Probe button, by hand. `ensureFreshQuota` (the gate) and `captureQuotaAfter` (run end) wrote in silence, which are the two moments somebody is watching. ⛔ `storeAndPublish` emits `quota.changed` — through `lastQuotaReading`, so a failed probe cannot bury good numbers — and **`QuotaPoller` no longer takes a listener**, so a fifth caller cannot opt out. ⛔ And `captureQuotaAfter` now claims `refreshNow` instead of calling `refreshUsage` bare, which is what made the line above's *never two terminals on one account* actually true. ⚠️ The other half was visible: `ageMs`/`stale` are stamped at send time and the strip read them off the payload, so a patched card froze at `read 2m ago` forever — `quotaFreshness` (shared, one threshold) recomputes both against a ticking clock. ⚠️ ⛔ **The 3h itself is not a fault**: the refresh clock was retired 2026-08-31 and an idle account keeps its reading by design — the probe interval is a *sweep* cadence, never a freshness promise. Not yet seen in flight.
