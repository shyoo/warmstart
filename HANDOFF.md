...
`npm run build` clean · `npm test` 388/388 · `npm run test:daemon` 124/124 · `npm run test:ui` 89/89 ·
...
src/daemon/            orchestratord. Runs as Electron-with-ELECTRON_RUN_AS_NODE, detached.
  ...
  db.ts                node:sqlite + numbered migrations (v10)
...
4. **Resident sessions** - one conversation per (worker, workspace), tasks borrowing it. The biggest
   ...
   ⛔ A dirty resident tree is never switched out from under its task. ⚠️ Topic scoring deferred: the
   gate returns a ranked list, so a score is a comparator later rather than a rewrite.
...
**R2 blocks the compaction reserve** (R3 closed). R1 and R6 change the cache clock; R5 needs a second subscription.
