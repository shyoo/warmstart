...
⛔ What each milestone *measured*, and what those measurements cost the design, is in
`transient_docs/changes_history.md`. Read it before re-deriving any of it.
...
`docs/cost-model.md` with the date and the CLI version, and delete the row from here.

**The instrument.** Multi Agent Controller meters *assistant turns* exactly from the transcript. Quota
measures *everything the account spent*. The gap between them is everything the CLI spent that never
reached a transcript — the auto-mode classifier, title generation, whatever else. That gap is the
measurement.
...
- **Deterministic scheduler; the LLM only on judgment events, and never inline.** A loop running every
  10s for weeks must not bill anything, and the fleet must survive the controller's own quota running
  out — including there being no controller at all.
...
- **Capabilities and objectives are data.** No `if (adapter === …)`, no `if (mode === …)`. Proved
  against three real CLIs in M5 and extended in M6 to adapters an operator declares in JSON.
