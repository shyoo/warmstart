...
the `user_version` a current database sits at — **66** as of 2026-09-11.
...
| `remote_devices` | one paired phone | ⛔ `token_hash` only — the token is shown once, at pairing, and never stored. Revoking **deletes** the row (t353): a revoked phone could do nothing, so keeping it only cluttered Paired devices. ⚠️ `revoked_at` remains for rows tombstoned by older builds, and every query still filters on it |
