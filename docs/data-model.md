...
the `user_version` a current database sits at — **65** as of 2026-09-11.
...
| `MessageEvent` | `worker.assigned` · `worker.switched` · `conversation.joined` — the closed set of concise system thread lines (migration 62, `task_messages.event`). ⛔ **One line per material change, never one per run.** `announceWorker` in `scheduler.ts` writes `worker.assigned` on a task's first `work` run, nothing when the next run lands on the same account, and `worker.switched … — <reason>` when it does not; the reason is read off `WorkerChoice.refusals` (the gate that turned the previous account away) and never inferred from prose. `conversation.joined` marks the one warm continuation that is an information boundary — a *borrowed* conversation; a task continuing its own session writes nothing. Everything the old *Started on X in <path> on <branch>* and *Controller routed this to X …* messages said now lives in `detail`, expandable and off the line |
