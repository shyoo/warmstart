...
- **GUI apps on macOS/Linux do not inherit shell profile PATH.** App bundles launched via desktop shells
  ...
  `which.ts` exports `augmentPath()`, `orchestratord` augments `process.env.PATH` at startup, and
  `spawnEnv()` prepends standard user bin directories. Project verification checks (`runChecks` in
