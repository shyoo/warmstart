...
| `report-only` | The deliverable is on the **thread**. Nothing is expected on the branch, so a clean branch with no commits completes rather than being handed back to a person. Not one thing more than `await-human` — strictly *less*. ⭐ And because the thread is the deliverable, `landCompletion` writes the run's **closing prose** (the peephole's run tail, last `REPORT_PROSE_CHARS`, minus lines the summary already holds) *after* the reported summary — `task_complete` describes its summary as one line, and a debate seat that obeyed the tool over its prompt was leaving a sentence where its position should be (t382). |
