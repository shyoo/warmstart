...
⛔ **Pieces never merge into the caller for it.** `merge-branch` frees a pooled slot holding the target by rescue-committing and detaching it — safe for a parked planner, but a conversation keeps talking in that slot. So a piece is filed `commit-and-verify`, cut from the caller's branch (`landingTarget`) in a worktree of its own, and the caller merges what it wants with `git` when `reportDelegationIfSettled` tells it every piece has settled. Both wait on `settled` edges, like a planner. A work task is `blocked` at once and its run ends. A conversation finishes its turn and then rests `blocked` rather than `awaiting_human` (`endConversationTurn`, t713: nothing is asked of the person). A person's message still gets a turn (`continueTask`), and `admit` never re-parks a conversation that is not already resting `blocked`. The last piece to settle admits it to `ready`; one that is mid-turn gets the report delivered, one resting on a question is requeued, and a stopped one is left alone.
