...
| `FleetStrip` `Workers` `ModelTable` `FleetSettings` | the fleet: per-account quota with its **age**, reset countdowns, live sessions; one two-column settings card per worker with inline model routing table (`ModelTable`) |
