...
| `Attention` `Questions` | the approvals/questions/quota-gate bar — one keystroke above the operator's work. A question card answers with options, prose, **and attachments**: the same [+] file/folder menu the composer has, because an agent's NEEDS DECISION lands here and "attach that folder" needs somewhere to happen (t521). A folder answered with is granted to the task exactly as a composed one; a folder alone, with no option or prose, counts as an answer. Both question lists discard older RPC replies after a newer refresh, so an answered question cannot return when a delayed reply arrives. A `task_split` body renders in the thread's markdown subset; a Plan & Execute handoff carries the whole executor instruction behind *Show full instruction*, collapsed past twelve lines (t693). Opening a task from the bar (Answer…/View…) lands on that task's own project thread — `routeForTask`, `lib/taskview.tsx` — never the Unassigned list; Unassigned is only for the genuinely project-less remainder (t699) |
...
- **Complete beside Send** once the task is `paused_user` (Stop first, then Complete). It is
  ...
  With nothing typed, the primary button on a stopped task reads **Resume** and on a failed task reads **Retry** (`task.resume`, which clears hold reason and assignee to re-enter `ready`); the
