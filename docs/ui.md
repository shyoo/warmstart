...
| `Tasks` `TaskThread` `thread/*` `Dependencies` | the board, one task's thread, and prerequisite edges. Opening a task lands at the **bottom** of its thread, where the recent conversation sits above the reply box — once per navigation, never per render, so a thread growing under a running agent does not yank back a reader who scrolled up (`lib/threadscroll.ts`) |
