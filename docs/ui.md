...
| `Tasks` `TaskThread` `thread/*` `Dependencies` | the board, one task's thread, and prerequisite edges. A project with open PRs renders a dedicated pending pull request banner (`.tasks-pr-banner`) above the filters with task links, PR URLs and an instant **Check merged PRs** action, and tasks with open deliveries carry a purple `PR #N` pill beside their title. Opening a task lands at the **bottom** of its thread, where the recent conversation sits above the reply box — once per navigation, never per render, so a thread growing under a running agent does not yank back a reader who scrolled up (`lib/threadscroll.ts`). ⭐ **And stays there.** A reader who has not scrolled away from the bottom is kept pinned to it as the thread grows — a scroll listener on the page's `.content` container tracks `isNearThreadBottom`, and a `useLayoutEffect` with no dependency list re-checks it after every render, the same "after every render, not on a loop" reasoning `useScrolledPast` already carries, and the same pinned-tail pattern `SessionStream` uses for its own live pane. Scroll away to read and the thread stops chasing you; scroll back to the bottom and it resumes. |
