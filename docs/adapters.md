...
| `openai-compatible` | models `gpt-5.2-codex*` | `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4-mini` — from the running CLI's `$CODEX_HOME/models_cache.json`, refreshed 2026-09-03 on codex-cli 0.151.0. Sol has `visibility: list` and accepts `low` through `ultra`; `gpt-reserve` and `codex-auto-review` are hidden, so neither is offered |
