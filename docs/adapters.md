...
| **Unattended authority** (`policy.headlessAuthority`) | ⛔ **full-user** (`bypassPermissions`) | ⛔ **full-user** (`--dangerously-skip-permissions`) | ⚠️ **sandboxed** (`--sandbox workspace-write`, widened by `grants.ts` to the shared `.git`) | ⛔ **full-user** *(unverified; conservative)* | ⛔ **full-user** (`--approval-mode never`) |
...
| *(Windows)* a worktree is a directory git can open | ⛔ **Not from inside WSL.** `<worktree>/.git` holds `gitdir: C:/Dev/…`, which git resolves *relatively*: `fatal: not a git repository: /mnt/c/…/ws1/C:/Dev/…`. Every workspace this app hands out is a worktree, so a muse worker could not have run one git command. ⭐ `GIT_DIR` + `GIT_WORK_TREE` fix it for `git` — ⛔ **and for nothing else.** Measured on t410 (2026-09-13): muse's own `edit_file`/`write_file` read `<worktree>/.git` themselves and refuse every edit with *cannot resolve workspace gitdir pointer: No such file or directory*, in every muse session this fleet has run in a worktree. The agent's answer was to overwrite the pointer with a `/mnt/c/…` spelling, which broke the slot for Windows git and cost the task its next dispatch. So the pointer is now written **relative** (`gitdir: ../../<repo>/.git/worktrees/wsN`), which git 2.54 (Windows) and 2.53 (WSL) both follow with no environment, and `ensureWorktreePointer` runs `git worktree repair` on any slot whose pointer no longer resolves. ⚠️ That muse's tools accept the relative form is inferred from their error, not yet measured against a live run |
