...
| Measured against | 2.1.223 | 1.1.20 | 0.155.1 (2026-09-20) | llama.cpp / Qwen3-Coder | 1.0.3 (Linux), 1.3.0 (Windows, 2026-09-19) |
...
| Can compact | ✔ — preemption interrupts the active stream turn, preserves its `aborted_tools` acknowledgement, then sends `/compact` | ⛔ | ⛔ *(conservative)* | ⛔ | ⛔ *(automatic thresholds only; no slash command)* |
| **Unattended authority** (`policy.headlessAuthority`) | ⛔ **full-user** (`bypassPermissions`) | ⛔ **full-user** (`--dangerously-skip-permissions`) | ⚠️ **sandboxed** (`--sandbox workspace-write`, widened by `grants.ts` to the shared `.git` and to a granted repository’s own) | ⛔ **full-user** *(unverified; conservative)* | ⛔ **full-user** (`--yolo` when the worker is set to Full user authority, else `--approval-mode never` + `--disable-sandbox`) |
...
| Warmstart MCP tools | ✔ | ⛔ global registration only | ✔ per-session CLI overrides (`-c mcp_servers.<name>...`) | ⛔ function calling in bridge | ⛔ `mcpServers` is per-**root** config, not per session |
...
| `openai-compatible` | models `gpt-5.2-codex*` | ChatGPT Codex: `gpt-6-astra`, `gpt-6-sol`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5` — refreshed from [OpenAI's Codex models documentation](https://learn.chatgpt.com/ja-JP/docs/models?surface=app) 2026-09-09 and verified live on codex-cli 0.156.0 on 2026-09-22. `gpt-6-astra` and `gpt-6-sol` are supported on ChatGPT Plus/Pro/Business with effort levels `low` through `ultra`. `gpt-5.4` and `gpt-5.4-mini` retired for ChatGPT sign-in on 2026-08-31; API-key Codex is unaffected. Astra and Sol accept `low` through `ultra`; `gpt-reserve` and `codex-auto-review` are hidden, so neither is offered |
...
| `openai-compatible` | `mcp: true`, because codex has MCP | ⭐ **Supported per-session via `-c mcp_servers.<name>...` CLI overrides (t618, 2026-09-22).** `codex mcp add` registers into shared config which cannot carry per-session identity, but `codex exec` accepts command-line configuration overrides. In `plan()`, `mcpConfig` translates into `-c mcp_servers.<name>.command=...`, `args=...`, `env.<k>=...`, and `-c mcp_servers.<name>.default_tools_approval_mode="approve"`, passing `WARMSTART_SESSION_ID` and `WARMSTART_TIER` without touching global config. `capabilities.mcp: true` is enabled. For non-MCP adapters (Antigravity, Muse), Debate mode uses terminal arbitration fallback (`DEBATE ROUND CONTINUE:` / `DEBATE ROUND CONVERGED:`) |
...
| `claude-code` | the quota probe is free because it spends no tokens | ⚠️ **Free of tokens, not of sessions.** Each `/usage` refresh opens a real interactive PTY that registers a session with the vendor's bridge, and those accumulate in the desktop app until archived by hand. Measured 2026-08-30: **150 probe sessions against 14 that did any work** in four days. The two-hour clock was retired 2026-08-31 (`ensureFreshQuota()` refreshes at the dispatch gate and when a run ends), and **that over-corrected**: an idle worker kept its last reading for as long as it sat unused, so cards read hours old under a 20-minute setting (t577, 2026-09-20). The idle refresh is back, bounded by the operator's own `idleProbeIntervalMinutes` — one terminal per account per interval, run last and one at a time — so the 150-versus-14 problem is capped by a number they chose rather than by neglect |
...
| `openai-compatible` | Codex tool calls and final answers are in the TUI stream | ⛔ **Tool calls were dropped and `turn.completed` omitted the final answer.** Measured 2026-09-23 on t665/t666: `codex exec --json` emits `item.started`/`item.completed` with `type: "command_execution"`, MCP calls, file edits, and reasoning. Dropping them left tool execution invisible in the Session TUI and thread disclosure, while `turn.completed` set `result.text: errorText` (null on success). Falling back to `backscroll` raced with `renderStream`'s scrollback append, causing initial planning commentary to be preserved and the final answer lost, leaving tasks in `awaiting_human` looking like broken stops. The decoder now decodes tool calls and extracts the final post-tool answer into `result.text`. |
...
| setting Muse to Full user authority makes `gh` use the host's login | ⛔ **Muse's private `XDG_CONFIG_HOME` also redirects `gh` to `<worker>/config/gh`.** On t682 (2026-09-24), Muse reported `gh` unauthenticated although the host and a following Codex run used the signed-in account. Reproduced on Windows: `gh auth status` with an empty XDG root said *not logged in*; with that same root plus `GH_CONFIG_DIR=%APPDATA%/GitHub CLI`, it reported the host's keyring account. The Muse adapter now sets `GH_CONFIG_DIR` to the host's original gh config location before replacing XDG, respecting an existing `GH_CONFIG_DIR` or host XDG override. This preserves Muse's own account isolation; full-user Muse runs can use the operator's gh login. |
| MCP tools ride `settings.json`, so Questions can too | ⛔ **The file is per-account and MCP identity is per-session.** Measured 2026-09-23 on 1.3.0 (t676), free via `--provider echo`: a scratch root whose `settings.json` (needs `schema_version`) holds one stdio server **is** honoured — the child spawned, its `env` arrived intact. But `WARMSTART_SESSION_ID` differs per pooled session, so two sessions on one account would share whichever identity was written last, and `ask_human`/`task_complete` would answer for the wrong run. `muse exec --help` carries no per-run MCP flag — the codex t618 escape (`-c mcp_servers.…`) has no muse equivalent — so `mcp` stays `false` and a muse run asks via `NEEDS DECISION:` text, which `turnend` parses back into a Question |
