...
| `openai-compatible` | models `gpt-5.2-codex*` | ChatGPT Codex: `gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5` — refreshed from [OpenAI's Codex models documentation](https://learn.chatgpt.com/ja-JP/docs/models?surface=app) 2026-09-09 and verified live on codex-cli 0.155.1 on 2026-09-20. `gpt-6-astra` is supported on ChatGPT Plus/Pro/Business with effort levels `low` through `ultra`. `gpt-5.4` and `gpt-5.4-mini` retired for ChatGPT sign-in on 2026-08-31; API-key Codex is unaffected. Astra and Sol accept `low` through `ultra`; `gpt-reserve` and `codex-auto-review` are hidden, so neither is offered |
