...
| inspect or debug a task from outside Warmstart | [`external-task-debugging.md`](external-task-debugging.md) — find the live database and trace a `t<number>` safely |
...
| [`external-task-debugging.md`](external-task-debugging.md) | Read-only SQLite queries for tracing a task, its runs, sessions, workspace, and logs from outside the app |
