...
- **Model capability classes (`constraints.modelClass`):** Tasks can request a capability tier (`high`, `med`, `low`) alongside Auto Model (`Auto Model (high)`, `Auto Model (med)`, `Auto Model (low)`). `chooseTarget` filters candidate models using `resolveModelClass(model, worker)`. Custom tiers can be configured per account in Workers tab (`workers.model_classes_json`), overriding built-in provider heuristics. If an account has no routable models matching the class, it is skipped; if no eligible account has a routable model in the class with quota, the task strictly holds (`no routable models in '<class>' class`) with no silent downgrade.
- **Routable model effort levels (`worker.modelEfforts`):** Workers settings support configuring reasoning effort levels per routable model in a table format (`workers.model_efforts_json`, migration 80). When a routable model is dispatched without explicit task effort constraints, the worker's per-model effort applies. The Workers tab displays each routable model annotated with its capability tier and configured effort level.
