 ...
 * machine (plan §19.3):
 *
 *  1. `WARMSTART_DATA_DIR` always points at a fresh temp directory. No test touches the real fleet.
...
export class Daemon {
  ...
  async start() {
    ...
    this.child = spawn(electronBinary(), [script], {
      env: { ...process.env, ELECTRON_RUN_AS_NODE: '1', WARMSTART_DATA_DIR: this.dataDir },
      stdio: ['ignore', 'pipe', 'pipe'],
      windowsHide: true
    })
    this.child.stdout.on('data', (d) => process.env.WARMSTART_TEST_VERBOSE && process.stdout.write(`[d] ${d}`))
