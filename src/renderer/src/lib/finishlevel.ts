...
/**
 * The levels the Commit button offers, and the two it does not.
 *
 ...
 *
 * ⛔ `report-only` for the first reason again, and more strongly: it is the level that says *nothing
 ...
 */
export const COMMIT_LEVELS: FinishPolicy[] = FINISH_ORDER.filter(
  (policy) => policy !== 'await-human' && policy !== 'custom' && policy !== 'report-only'
)

/**
 * The levels the Land button offers: the ones the **tool** acts on.
 *
 * ⛔ Derived from `policyLands`, for the reason above and one more of its own. Landing a branch under
 * `commit-only` or `commit-and-verify` is a button that does nothing — those levels leave the branch
 ...
 */
export const LAND_LEVELS: FinishPolicy[] = FINISH_ORDER.filter(policyLands)

/**
 * The levels a settle-it menu offers when the task holds the trunk lease.
 *
 ...
 */
export function commitLevelsForMode(mode: WorkspaceMode): FinishPolicy[] {
  return mode === 'trunk' ? COMMIT_LEVELS.filter(policyOfferedInTrunk) : COMMIT_LEVELS
}

export function landLevelsForMode(mode: WorkspaceMode): FinishPolicy[] {
  return mode === 'trunk' ? LAND_LEVELS.filter(policyOfferedInTrunk) : LAND_LEVELS
}
 ...
 * level left: pushing the target is still landing.
 */
 ...
 * written for — hiding levels on a guess would be worse than showing them.
 */
 ...
 * substitute is the level that does the least: commit, and leave the branch where it is. Falling back
...
/**
 * ⚠️ The fleet default, which lands by construction. A Land button whose level does not land is a
 * button that does nothing, so the fallback here has to be a landing level rather than the quietest
 * one.
 */
export const LAND_FALLBACK: FinishPolicy = DEFAULT_FLEET_FINISH

/**
 * Which level a settle-it button starts on: the task's own, else the project's, else the fleet's.
 *
 * ⭐ **The bug this closes (t283).** The Commit control was a picker with no value, so its menu
 * opened on the first level in the ladder — `commit-only` — and an operator on a project configured
 * for `commit·verify·merge` was offered the one level that leaves the work sitting on the branch,
 ...
 * the remaining tiers are read in their usual order: a level the task already carries wins, because
 ...
 *
 * ⚠️ `offered` is the list the menu actually shows, so the answer is always a level that can be
...
/**
 * Where the level on a settle-it button came from, as the half-sentence the card prints after it.
 *
 ...
 */
export function levelOrigin(
  taskPolicy: FinishPolicyChoice,
  inherited: { policy: FinishPolicy; source: string } | null | undefined,
  chosen: FinishPolicy
): string {
  if (taskPolicy !== 'inherit' && taskPolicy === chosen) return 'the level this task already carries'
