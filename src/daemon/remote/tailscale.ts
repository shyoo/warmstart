...
export interface TailscaleInfo { installed: boolean; hostname: string | null; ipv4: string | null; certAvailable: boolean; certPath: string | null; keyPath: string | null }
...
export async function tailscaleInfo(): Promise<TailscaleInfo> {
  ...
  if (!binary) return { installed: false, hostname: null, ipv4: null, certAvailable: false, certPath: null, keyPath: null }
  try {
    ...
    if (!hostname) return { installed: true, hostname: null, ipv4, certAvailable: false, certPath: null, keyPath: null }
    ...
    return { installed: true, hostname, ipv4, certAvailable, certPath: certAvailable ? certPath : null, keyPath: certAvailable ? keyPath : null }
  } catch {
    return { installed: true, hostname: null, ipv4: null, certAvailable: false, certPath: null, keyPath: null }
  }
}
